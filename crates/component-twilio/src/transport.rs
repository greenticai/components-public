...
/// Twilio's own guidance is that a call should not hang; 30s is the extension's
...
/// Classify a Twilio response. A non-2xx is a routable failure, not a crash.
///
...
mod tests {
    ...
    /// The error text must be able to carry Twilio's message without ever
    ...
    fn a_non_2xx_reports_the_status_and_a_bounded_body_snippet() {
        ...
        .unwrap_err();
        assert!(err.starts_with("Twilio request failed (401):"));
        assert!(
            err.len() < 320,
