...
/// Calendly's own guidance is that a call should not hang; 30s is the extension's
...
/// Classify a Calendly response. A non-2xx is a routable failure, not a crash.
///
...
pub fn check(resp: Response) -> Result<Vec<u8>, String> {
    ...
    Err(format!(
        "Calendly request failed ({}): {snippet}",
        resp.status
    ))
}
...
mod tests {
    ...
    /// The error text must be able to carry Calendly's message without ever
    ...
    fn a_non_2xx_reports_the_status_and_a_bounded_body_snippet() {
        ...
        .unwrap_err();
        assert!(err.starts_with("Calendly request failed (401):"));
        assert!(
            err.len() < 320,
