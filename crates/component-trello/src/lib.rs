...
//! Trello board, list and card operations for Greentic flows.
//!
//! `greentic.trello` ships its tools as a DESIGN extension, reachable to an
...
pub fn dispatch(op: &str, input: &Value) -> Value {
    match op {
        "trello_cards" => ops::trello_cards(input),
        "trello_lists" => ops::trello_lists(input),
        "trello_boards" => ops::trello_boards(input),
        "trello_checklists" => ops::trello_checklists(input),
        "trello_labels" => ops::trello_labels(input),
        "trello_comments" => ops::trello_comments(input),
        "trello_attachments" => ops::trello_attachments(input),
        "trello_members" => ops::trello_members(input),
...
pub fn op_summary(op: &str) -> &'static str {
    match op {
        "trello_cards" => "Create, read, move or archive cards",
        "trello_lists" => "Create, read or archive lists",
        "trello_boards" => "Read and manage boards",
        "trello_checklists" => "Manage checklists and their items",
        "trello_labels" => "Create and apply labels",
        "trello_comments" => "Read and post card comments",
        "trello_attachments" => "Attach files and links to cards",
        "trello_members" => "Read and assign board members",
        _ => "Unknown operation",
    }
}
...
pub fn i18n_keys() -> Vec<String> {
    [
        "trello.schema.input.title",
        "trello.schema.input.description",
        "trello.schema.input.token.title",
        "trello.schema.input.token.description",
        "trello.schema.output.title",
        "trello.schema.output.description",
        "trello.schema.output.ok.title",
        "trello.schema.output.ok.description",
        "trello.schema.output.error.title",
        "trello.schema.output.error.description",
    ]
    .iter()
    .map(|k| (*k).to_string())
    .collect()
...
impl node::Guest for Component {
    fn describe() -> node::ComponentDescriptor {
        node::ComponentDescriptor {
            ...
            summary: Some("Trello board, list and card operations for Greentic flows".to_string()),
