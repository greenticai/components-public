//! `stripe_products` tool domain — pure HTTP-call building and response
//! normalization for Stripe product operations (create/list/get/update). No
//! WIT imports — this module is fully host-testable; the actual
//! `extension-host/http` invocation and `describe()` tool metadata live in
//! `lib.rs` / `tool_meta.rs`. See [`crate::tools::customers`] for the
//! template this domain follows.

// Copied verbatim from the greentic.stripe design extension. The only edit is
// this attribute: the tool-metadata tables and several op enums exist for the
// TOOL surface and are unused by the node surface. Silencing it here keeps the
// rest of the file diffable against its source.
#![allow(dead_code)]
use serde::Deserialize;
use serde_json::{Value, json};

use crate::client::{HttpCall, Method};

/// Stripe product operation selected by the `operation` input field.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProductOp {
    Create,
    List,
    Get,
    Update,
}

/// Raw `stripe_products` tool input, deserialized from the model-supplied
/// `args_json`.
#[derive(Debug, Deserialize)]
struct ProductsInput {
    operation: ProductOp,
    #[serde(default)]
    id: Option<String>,
    /// Pass-through create/update body (e.g. `name`, `description`,
    /// `active`). Required for `create` and `update`; Stripe itself requires
    /// `name` within it for `create`.
    #[serde(default)]
    params: Option<Value>,
    #[serde(default)]
    limit: Option<u32>,
    #[serde(default)]
    active: Option<bool>,
}

/// Build the Stripe REST [`HttpCall`] for a `stripe_products` invocation.
///
/// Parses `args_json` into a [`ProductsInput`], validates the fields
/// required by the selected [`ProductOp`], and returns the resulting
/// request. On missing input or a missing required field, returns `Err`
/// naming the field.
pub fn build_call(args_json: &str) -> Result<HttpCall, String> {
    let input: ProductsInput =
        serde_json::from_str(args_json).map_err(|err| format!("invalid input: {err}"))?;
    match input.operation {
        ProductOp::Create => build_create(&input),
        ProductOp::List => Ok(build_list(&input)),
        ProductOp::Get => build_get(&input),
        ProductOp::Update => build_update(&input),
    }
}

/// Extract just the `operation` field from `args_json`, without validating
/// the other fields `build_call` requires. `lib.rs` calls this after
/// `build_call` succeeds so it knows which [`normalize`] arm to run.
pub fn parse_operation(args_json: &str) -> Result<ProductOp, String> {
    #[derive(Deserialize)]
    struct OperationOnly {
        operation: ProductOp,
    }
    serde_json::from_str::<OperationOnly>(args_json)
        .map(|parsed| parsed.operation)
        .map_err(|err| format!("invalid input: {err}"))
}

fn build_create(input: &ProductsInput) -> Result<HttpCall, String> {
    let params = super::require_params(input.params.as_ref(), "params")?;
    Ok(HttpCall {
        method: Method::Post,
        path: "/products".to_string(),
        query: Vec::new(),
        body: Some(params),
    })
}

fn build_list(input: &ProductsInput) -> HttpCall {
    let mut query = Vec::new();
    if let Some(limit) = input.limit {
        query.push(("limit".to_string(), limit.to_string()));
    }
    if let Some(active) = input.active {
        query.push(("active".to_string(), active.to_string()));
    }
    HttpCall {
        method: Method::Get,
        path: "/products".to_string(),
        query,
        body: None,
    }
}

fn build_get(input: &ProductsInput) -> Result<HttpCall, String> {
    let id = super::require_field(input.id.as_deref(), "id")?;
    Ok(HttpCall {
        method: Method::Get,
        path: format!("/products/{id}"),
        query: Vec::new(),
        body: None,
    })
}

fn build_update(input: &ProductsInput) -> Result<HttpCall, String> {
    let id = super::require_field(input.id.as_deref(), "id")?;
    let params = super::require_params(input.params.as_ref(), "params")?;
    Ok(HttpCall {
        method: Method::Post,
        path: format!("/products/{id}"),
        query: Vec::new(),
        body: Some(params),
    })
}

/// Map a raw Stripe REST response body to the compact shape returned to the
/// model, based on the [`ProductOp`] that produced it.
pub fn normalize(op: ProductOp, raw: &[u8]) -> Result<Value, String> {
    match op {
        ProductOp::List => normalize_list(raw),
        ProductOp::Create | ProductOp::Get | ProductOp::Update => normalize_record(raw),
    }
}

/// Pull `{id,name,active}` out of a single Stripe product object,
/// defensively — every field falls back to `null` rather than panicking.
fn product_fields(value: &Value) -> Value {
    json!({
        "id": value.get("id").cloned().unwrap_or(Value::Null),
        "name": value.get("name").cloned().unwrap_or(Value::Null),
        "active": value.get("active").cloned().unwrap_or(Value::Null),
    })
}

/// Normalize a single-product response (create/get/update) to
/// `{id,name,active}`.
fn normalize_record(raw: &[u8]) -> Result<Value, String> {
    let value: Value =
        serde_json::from_slice(raw).map_err(|err| format!("invalid product response: {err}"))?;
    Ok(product_fields(&value))
}

/// Normalize a list response (Stripe's `data[]` list shape) to
/// `{total, results:[{id,name,active}]}`. `total` is the count of mapped
/// `results`, not a value read from the response body.
fn normalize_list(raw: &[u8]) -> Result<Value, String> {
    let value: Value =
        serde_json::from_slice(raw).map_err(|err| format!("invalid list response: {err}"))?;
    let results: Vec<Value> = value
        .get("data")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .map(product_fields)
        .collect();
    Ok(json!({ "total": results.len(), "results": results }))
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn create_builds_post_with_params_body() {
        let call = build_call(r#"{"operation":"create","params":{"name":"Widget"}}"#).unwrap();
        assert!(matches!(call.method, Method::Post));
        assert_eq!(call.path, "/products");
        assert_eq!(call.body.as_ref().unwrap()["name"], "Widget");
    }

    #[test]
    fn create_missing_params_names_field() {
        let err = build_call(r#"{"operation":"create"}"#).unwrap_err();
        assert!(err.contains("params"));
    }

    #[test]
    fn list_builds_get_with_limit_and_active_query() {
        let call = build_call(r#"{"operation":"list","limit":10,"active":true}"#).unwrap();
        assert!(matches!(call.method, Method::Get));
        assert_eq!(call.path, "/products");
        assert!(call.query.iter().any(|(k, v)| k == "limit" && v == "10"));
        assert!(call.query.iter().any(|(k, v)| k == "active" && v == "true"));
    }

    #[test]
    fn list_with_no_filters_has_empty_query() {
        let call = build_call(r#"{"operation":"list"}"#).unwrap();
        assert!(call.query.is_empty());
    }

    #[test]
    fn get_builds_get_with_id_path() {
        let call = build_call(r#"{"operation":"get","id":"prod_1"}"#).unwrap();
        assert!(matches!(call.method, Method::Get));
        assert_eq!(call.path, "/products/prod_1");
    }

    #[test]
    fn get_missing_id_names_field() {
        let err = build_call(r#"{"operation":"get"}"#).unwrap_err();
        assert!(err.contains("id"));
    }

    #[test]
    fn update_builds_post_with_id_and_params() {
        let call = build_call(r#"{"operation":"update","id":"prod_1","params":{"active":false}}"#)
            .unwrap();
        assert!(matches!(call.method, Method::Post));
        assert_eq!(call.path, "/products/prod_1");
        assert_eq!(call.body.as_ref().unwrap()["active"], false);
    }

    #[test]
    fn update_missing_params_names_field() {
        let err = build_call(r#"{"operation":"update","id":"prod_1"}"#).unwrap_err();
        assert!(err.contains("params"));
    }

    #[test]
    fn normalize_record_extracts_fields() {
        let raw = br#"{"id":"prod_1","name":"Widget","active":true}"#;
        let out = normalize(ProductOp::Get, raw).unwrap();
        assert_eq!(out["id"], "prod_1");
        assert_eq!(out["name"], "Widget");
        assert_eq!(out["active"], true);
    }

    #[test]
    fn normalize_record_handles_missing_fields_without_panicking() {
        let raw = br#"{"id":"prod_1"}"#;
        let out = normalize(ProductOp::Create, raw).unwrap();
        assert_eq!(out["id"], "prod_1");
        assert_eq!(out["name"], Value::Null);
        assert_eq!(out["active"], Value::Null);
    }

    #[test]
    fn normalize_list_maps_data_array() {
        let raw = br#"{"data":[{"id":"prod_1","name":"Widget","active":true}],"has_more":false}"#;
        let out = normalize(ProductOp::List, raw).unwrap();
        assert_eq!(out["total"], 1);
        assert_eq!(out["results"][0]["id"], "prod_1");
        assert_eq!(out["results"][0]["name"], "Widget");
    }

    #[test]
    fn normalize_list_handles_empty_data() {
        let raw = br#"{"data":[]}"#;
        let out = normalize(ProductOp::List, raw).unwrap();
        assert_eq!(out["total"], 0);
        assert_eq!(out["results"].as_array().unwrap().len(), 0);
    }

    #[test]
    fn parse_operation_extracts_op_ignoring_other_fields() {
        assert_eq!(
            parse_operation(r#"{"operation":"get","id":"prod_1"}"#),
            Ok(ProductOp::Get)
        );
        assert!(parse_operation(r#"{"operation":"nope"}"#).is_err());
        assert!(parse_operation("{not json").is_err());
    }
}
