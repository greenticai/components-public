...
/// A self-contained request. The design extension builds path + query + body in
/// its pure modules and hands the pieces to `lib.rs`; a component needs one
/// struct to carry them across the host seam, so it is declared here.
...
/// Stripe's own guidance is that a call should not hang; 30s is the extension's
...
/// Classify a Stripe response. A non-2xx is a routable failure, not a crash.
///
...
mod tests {
    ...
    /// The error text must be able to carry Stripe's message without ever
    ...
    fn a_non_2xx_reports_the_status_and_a_bounded_body_snippet() {
        ...
        .unwrap_err();
        assert!(err.starts_with("Stripe request failed (401):"));
        assert!(
            err.len() < 320,
