...
//! Tavily web search and extraction for Greentic flows.
//!
//! `greentic.tavily` ships its tools as a DESIGN extension, reachable to an
...
pub const OPS: &[&str] = &["tavily_search", "tavily_extract"];
...
pub fn dispatch(op: &str, input: &Value) -> Value {
    match op {
        "tavily_search" => ops::tavily_search(input),
        "tavily_extract" => ops::tavily_extract(input),
...
pub fn op_summary(op: &str) -> &'static str {
    match op {
        "tavily_search" => "Search the web and return ranked results",
...
pub fn i18n_keys() -> Vec<String> {
    [
        "tavily.schema.input.title",
        "tavily.schema.input.description",
        "tavily.schema.input.token.title",
        "tavily.schema.input.token.description",
        "tavily.schema.output.title",
        "tavily.schema.output.description",
        "tavily.schema.output.ok.title",
        "tavily.schema.output.ok.description",
        "tavily.schema.output.error.title",
        "tavily.schema.output.error.description",
    ]
    .iter()
    .map(|k| (*k).to_string())
    .collect()
...
impl node::Guest for Component {
    fn describe() -> node::ComponentDescriptor {
        node::ComponentDescriptor {
            ...
            summary: Some("Tavily web search and extraction for Greentic flows".to_string()),
