...
//! Jira issue tracking for Greentic flows.
//!
//! `greentic.jira` ships its tools as a DESIGN extension, reachable to an
...
pub fn dispatch(op: &str, input: &Value) -> Value {
    match op {
        "jira_issues" => ops::jira_issues(input),
        "jira_comments" => ops::jira_comments(input),
        "jira_projects" => ops::jira_projects(input),
        "jira_boards" => ops::jira_boards(input),
        "jira_sprints" => ops::jira_sprints(input),
        "jira_worklogs" => ops::jira_worklogs(input),
        "jira_attachments" => ops::jira_attachments(input),
        "jira_users" => ops::jira_users(input),
...
pub fn op_summary(op: &str) -> &'static str {
    match op {
        "jira_issues" => "Create, read, update and search issues",
        "jira_comments" => "Read and post issue comments",
        "jira_projects" => "Read projects",
        "jira_boards" => "Read agile boards",
        "jira_sprints" => "Read and manage sprints",
        "jira_worklogs" => "Read and log work",
        "jira_attachments" => "Read and add attachments",
        "jira_users" => "Read and search users",
        _ => "Unknown operation",
    }
}
...
pub fn i18n_keys() -> Vec<String> {
    [
        "jira.schema.input.title",
        "jira.schema.input.description",
        "jira.schema.input.token.title",
        "jira.schema.input.token.description",
        "jira.schema.output.title",
        "jira.schema.output.description",
        "jira.schema.output.ok.title",
        "jira.schema.output.ok.description",
        "jira.schema.output.error.title",
        "jira.schema.output.error.description",
    ]
    .iter()
    .map(|k| (*k).to_string())
    .collect()
...
impl node::Guest for Component {
    fn describe() -> node::ComponentDescriptor {
        node::ComponentDescriptor {
            ...
            summary: Some("Jira issue tracking for Greentic flows".to_string()),
...
mod tests {
    ...
    fn a_missing_credential_is_named_first() {
        ...
        assert_eq!(out["ok"], false);
        assert!(out["error"].as_str().unwrap().contains("email"));
    }
