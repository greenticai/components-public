...
/// A self-contained request. The design extension builds URL + body in its pure
/// modules and hands the pieces to `lib.rs`; a component needs one struct to
...
/// Firecrawl's own guidance is that a call should not hang; 30s is the extension's
...
/// Classify a Firecrawl response. A non-2xx is a routable failure, not a crash.
///
...
pub fn check(resp: Response) -> Result<Vec<u8>, String> {
    ...
    Err(format!(
        "Firecrawl request failed ({}): {snippet}",
        resp.status
    ))
}
...
mod tests {
    ...
    /// The error text must be able to carry Firecrawl's message without ever
    ...
    fn a_non_2xx_reports_the_status_and_a_bounded_body_snippet() {
        ...
        .unwrap_err();
        assert!(err.starts_with("Firecrawl request failed (401):"));
        assert!(
            err.len() < 320,
