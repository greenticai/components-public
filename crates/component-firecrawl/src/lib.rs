...
//! Firecrawl web scraping and search for Greentic flows.
//!
//! `greentic.firecrawl` ships its tools as a DESIGN extension, which makes them
...
//!
//! `input`, `output` and `tool_meta` are the extension's modules copied
...
//! runs it as a node. Only `transport` and `ops` are new — the host seam the
//! extension gets from `extension-host/http` + `extension-host/secrets` has to
//! come from the guest crate instead.
...
const COMPONENT_ID: &str = "firecrawl";
#[cfg_attr(not(target_arch = "wasm32"), allow(dead_code))]
...
pub fn dispatch(op: &str, input: &Value) -> Value {
    match op {
        "web_scrape" => ops::web_scrape(input),
        "web_extract" => ops::web_extract(input),
        "web_screenshot" => ops::web_screenshot(input),
        "web_search" => ops::web_search(input),
        "web_crawl_start" => ops::web_crawl_start(input),
        "web_crawl_result" => ops::web_crawl_result(input),
        "browser_task" => ops::browser_task(input),
...
pub fn op_summary(op: &str) -> &'static str {
    match op {
        "web_scrape" => "Fetch a page as markdown, html or links",
        "web_extract" => "LLM-guided structured extraction from a page",
        "web_screenshot" => "Screenshot a page",
        "web_search" => "Web search, returning ranked results",
        "web_crawl_start" => "Start a site crawl and return a job id",
        "web_crawl_result" => "Poll a crawl job by id",
        "browser_task" => "Run an interactive browser action sequence",
        _ => "Unknown operation",
    }
}

/// The QA spec for a component that asks nothing at wizard time. The one input
/// an operator must supply is the credential, and that is declared on the node
/// type's `config_schema` where it is authored per node — asking again in a
/// setup wizard would create a second place to put it with no rule for which
/// wins.
...
pub fn i18n_keys() -> Vec<String> {
    [
        "firecrawl.schema.input.title",
        "firecrawl.schema.input.description",
        "firecrawl.schema.input.token.title",
        "firecrawl.schema.input.token.description",
        "firecrawl.schema.output.title",
        "firecrawl.schema.output.description",
        "firecrawl.schema.output.ok.title",
        "firecrawl.schema.output.ok.description",
        "firecrawl.schema.output.error.title",
        "firecrawl.schema.output.error.description",
    ]
    .iter()
    .map(|k| (*k).to_string())
    .collect()
...
impl node::Guest for Component {
    fn describe() -> node::ComponentDescriptor {
        node::ComponentDescriptor {
            ...
            summary: Some("Firecrawl web scraping and search for Greentic flows".to_string()),
