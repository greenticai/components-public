//! Greentic events-triggers design extension.
//!
//! This extension ships the canonical `nodeTypes` descriptors for
//! event-based trigger nodes (timer, SMS, email) so they appear in
//! the Greentic Designer node palette as first-class trigger primitives.
//!
...
impl manifest::Guest for Component {
    fn get_identity() -> types::ExtensionIdentity {
        types::ExtensionIdentity {
            id: "greentic.events-triggers".to_string(),
    ...
    fn get_offered() -> Vec<types::CapabilityRef> {
        vec![types::CapabilityRef {
            id: "greentic:events-triggers/trigger-nodes".into(),
            version: "1.0.0".into(),
        }]
    }
...
impl wit_tools::Guest for Component {
    ...
    fn invoke_tool(name: String, _args_json: String) -> Result<String, types::ExtensionError> {
        Err(types::ExtensionError::InvalidInput(format!(
            "events-triggers exposes no tools (got '{name}')"
        )))
    }
}
    ...
    fn validate_content(content_type: String, _content_json: String) -> validation::ValidateResult {
        validation::ValidateResult {
            ...
            diagnostics: vec![types::Diagnostic {
                ...
                message: format!(
                    "events-triggers does not validate content types (got '{content_type}')"
                ),
                path: None,
            }],
        }
...
impl knowledge::Guest for Component {
    ...
    fn get_entry(id: String) -> Result<knowledge::Entry, types::ExtensionError> {
        Err(types::ExtensionError::InvalidInput(format!(
            "events-triggers exposes no knowledge entries (got '{id}')"
        )))
    }
